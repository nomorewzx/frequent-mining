...
public class AlgoFPMaxNew {
    ...
    public Itemsets runAlgorithm(List<List<Integer>> transactionSet, String output, double minsupp) throws IOException {
        ...
        originalMapSupport = scanDatabaseToDetermineFrequencyOfSingleItems(transactionSet);
        ...
        for (List<Integer> transaction : transactionSet) {
            ...
            List<Integer> filteredTransaction = transaction.stream().filter(item -> originalMapSupport.get(item) >= minSupportRelative).collect(Collectors.toList());

            Collections.sort(filteredTransaction, comparatorOriginalOrder);
...
//		======= DEBUG ========
     ...
     */
    public Map<Integer, Integer> scanDatabaseToDetermineFrequencyOfSingleItems(List<List<Integer>> transactionSet) {
        Map<Integer, Integer> mapSupport = new HashMap<Integer, Integer>();
        transactionCount += transactionSet.size();

        transactionSet.stream().forEach(itemSet -> itemSet.stream().forEach(item -> {
            Integer itemCount = mapSupport.get(item);
            if (itemCount == null) {
                mapSupport.put(item, 1);
            } else {
                mapSupport.put(item, ++itemCount);
            }
        }));
